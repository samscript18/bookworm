...
const ReviewCard = ({ review, isDark }: { review: (typeof reviews)[number]; isDark: boolean }) => (
	<View className="rounded-2xl px-4 py-3 mb-3" style={{ backgroundColor: isDark ? "#141821" : "#FFFFFF", borderWidth: 1, borderColor: isDark ? "#2A2D38" : "#ECECF0" }}>
		<View className="flex-row items-start justify-between">
			<View className="flex-row items-center flex-1 pr-3">
				<Image source={{ uri: review.avatar }} className="w-10 h-10 rounded-full mr-3 bg-gray-200" />
		...
		<TouchableOpacity className="flex-row items-center mt-4">
			...
			<Text className="text-[#7F3DFF] text-[13px] ml-1.5">Helpful ({review.helpful})</Text>
		</TouchableOpacity>
	</View>
);
