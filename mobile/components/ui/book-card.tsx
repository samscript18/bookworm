				...
				<TouchableOpacity className="w-full mb-4">
					<View className="relative w-full">
						...
						<View className="absolute top-2 right-2 px-2 py-1 rounded-md" style={{ backgroundColor: theme.colors.primary }}>
							<Text className="font-manrope text-white text-xs font-bold capitalize">{book.tags[0]}</Text>
						</View>
					</View>
					<View className="w-full p-3 rounded-b-xl gap-y-2 shadow-sm" style={{ backgroundColor: theme.mode === "dark" ? "#141821" : "#FFFFFF" }}>
						<Text className="font-manrope font-semibold mt-2" style={{ color: theme.colors.textPrimary }} numberOfLines={1}>
							{book.title}
						</Text>
						<View className="flex-row items-center mt-1">
							<Ionicons name="star" size={14} color={theme.colors.primary} />
							<Text className="font-manrope text-xs ml-1" style={{ color: theme.colors.textPrimary }}>
				...
				<TouchableOpacity className="w-full mb-4 px-4">
					<View className="flex-row rounded-xl overflow-hidden" style={{ backgroundColor: theme.mode === "dark" ? "#141821" : "#FFFFFF" }}>
						...
						<View className="flex-1 px-3 py-2 justify-between">
							<View>
								<Text className="font-manrope font-semibold" style={{ color: theme.colors.textPrimary }} numberOfLines={2}>
									{book.title}
								</Text>
								<Text className="font-manrope text-xs mt-1 capitalize" style={{ color: theme.colors.textSecondary }} numberOfLines={1}>
									{book.author}
								</Text>
							</View>
